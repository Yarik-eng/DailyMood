...
# Запустити без збірки (якщо образ вже є)
docker compose up
...
# Подивитись список контейнерів
docker ps

# Зайти всередину контейнера (для дебагу)
docker exec -it dailymood-app bash
```

- `http://127.0.0.1:5000` — застосунок
- SQLite зберігається у volume `./data -> /app/data`
